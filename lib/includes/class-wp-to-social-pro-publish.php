	...
	private function validate( $post_id, $action ) {
		...
		if ( ! in_array( $action, $supported_actions, true ) ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: Action */
					__( 'The %s action is not supported.', 'postiz-auto-poster' ),
					$action
				)
			);
		}
		...
		if ( ! $post ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: Post ID */
					__( 'No WordPress Post could be found for Post ID %s', 'postiz-auto-poster' ),
					$post_id
				)
			);
		}
			...
			if ( isset( $status['error'] ) && is_wp_error( $status['error'] ) ) {
				...
				$logs[]      = array(
					...
					'result_message' => sprintf(
						...
						'%1$s: %2$s',
						__( 'Plugin Error', 'postiz-auto-poster' ),
