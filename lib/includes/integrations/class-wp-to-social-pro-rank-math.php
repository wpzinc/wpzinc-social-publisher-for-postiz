...
class WP_To_Social_Pro_Rank_Math {
	...
	public function register_status_tags( $tags ) {
		...
		return array_merge(
			...
			array(
				'rank_math' => array(
					'{rank_math_meta_title}'       => __( 'Meta Title', 'postiz-auto-poster' ),
					'{rank_math_meta_description}' => __( 'Meta Description', 'postiz-auto-poster' ),
				),
			)
		);
	...
	private function get_searches_replacements( $post, $author ) {
		 ...
		 * @param   WP_User     $author                 WordPress User (Author of the Post).
		 */
		$searches_replacements = apply_filters( 'postiz_auto_poster_publish_register_rank_math_searches_replacements', $searches_replacements, $post, $author );
