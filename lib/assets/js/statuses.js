...
function wpToSocialProReinitAutosize() {
	(function ($) {
		...
		if (
			$('.wpzinc-autosize-js', $(wp_to_social_pro.status_form)).length ===
			0
		) {
			return;
		}
...
function wpToSocialProInitTags() {
	(function ($) {
		...
		$('select.tags', $(wp_to_social_pro.status_form)).each(function () {
			$(this).on('change.wpzinc-social-publisher-pro', function () {
				// Insert tag into required textarea.
				let tag = $(this).val();
				const textarea = $(this).attr('data-textarea');
				const option = $('option:selected', $(this));
				const status = $(this).closest(
					'.wpzinc-social-publisher-pro-status-form'
				);
 ...
 */
function wpToSocialProInitSelectize(
	selector,
	profile_id,
	action,
	status_index
...
function wpToSocialProUpdateScheduleOptions(action) {
	(function ($) {
		...
		if (
			$('select.schedule', $(wp_to_social_pro.status_form)).length === 0
		) {
			return;
		}
		...
		switch ($('select.schedule', $(wp_to_social_pro.status_form)).val()) {
			case 'custom':
				...
				$(
					...
					$(wp_to_social_pro.status_form)
				).show();
				$('span.relative', $(wp_to_social_pro.status_form)).hide();
				$('span.custom', $(wp_to_social_pro.status_form))
					.text('after ' + action)
					.show();
 ...
 */
function wpToSocialProUpdatePostTypeOptions(profile) {
	(function ($) {
		// Disable all options.
		$(
			'option',
			$('select.post_type', $(wp_to_social_pro.status_form))
		).attr('disabled', true);
		...
		if (
			$('select.post_type', $(wp_to_social_pro.status_form)).val() ===
			'disabled'
		) {
			$('select.post_type', $(wp_to_social_pro.status_form)).val(
				$(
 ...
 * @since 	6.0.0
 */
function wpToSocialProUpdateStatusSections() {
	(function ($) {
		// Hide all sections.
		$('.wpzinc-option.link', $(wp_to_social_pro.status_form)).hide();
		$('.wpzinc-option.images', $(wp_to_social_pro.status_form)).hide();
		$('.wpzinc-option.pinterest', $(wp_to_social_pro.status_form)).hide();
		$(
		...
		switch ($('select.post_type', $(wp_to_social_pro.status_form)).val()) {
			case 'link':
				$(
					'.wpzinc-option.link',
					$(wp_to_social_pro.status_form)
 ...
 */
function wpToSocialProEditStatus(
	profile_id,
	profile,
	action,
	status_index,
	...
	wpToSocialProPopulateStatusForm(
		profile_id,
		profile,
		action,
		status_index,
	...
	wpToSocialProUpdatePostTypeOptions(profile);
	...
	wpToSocialProUpdateStatusSections();
...
function wpToSocialProDeleteStatus(profile_id, action, status_index) {
	(function ($) {
		// Confirm deletion.
		const result = confirm(wp_to_social_pro.delete_status_message);
		if (!result) {
			return;
		}
...
function wpToSocialProUpdateStatus(profile_id, action, status_index) {
	(function ($) {
		...
		$.each(
			...
			function (index, field) {
				...
				field.name = field.name.replace(
					wp_to_social_pro.plugin_name + '_',
					''
				);
		...
		$.ajax({
			...
			error(xhr) {
				alert(
					'wpToSocialProUpdateStatus(): error: ' +
						xhr.status +
						' ' +
						xhr.statusText
				);
 ...
 * @param {Object} status       Status.
 */
function wpToSocialProPopulateStatusForm(
	profile_id,
	profile,
	action,
	status_index,
...
function wpToSocialProDisplayStatusForm(profile_id, action, status_index) {
	(function ($) {
		...
		$('td', status_form_container_row).append(
			$(wp_to_social_pro.status_form).removeClass('hidden')
		);
...
function wpToSocialProSaveAndHideStatusForm() {
	(function ($) {
		...
		if (
			$('div.statuses div.wpzinc-social-publisher-pro-status-form')
				.length > 0
		) {
			wpToSocialProUpdateStatus(
				$(
					'div.statuses tr.status-form-container div.wpzinc-social-publisher-pro-status-form'
				).data('profile-id'),
				$(
					'div.statuses tr.status-form-container div.wpzinc-social-publisher-pro-status-form'
				).data('action'),
		...
		$(wp_to_social_pro.status_form_container).append(
			$(wp_to_social_pro.status_form)
		);
	})(jQuery);
}
		...
		$.ajax({
			...
			error(xhr) {
				wpzinc_modal_show_error_message_and_exit(
					'wpToSocialProSaveStatuses(): error: ' +
						xhr.status +
						' ' +
						xhr.statusText
				);
			...
			success(result) {
				...
				wpzinc_modal_show_success_and_exit(
					wp_to_social_pro.save_statuses_modal.title_success
				);
			},
		});
	})(jQuery);
 ...
 */
function wpToSocialProSavePostStatuses(
	post_id,
	override,
	featured_image,
	additional_images
...
let wpToSocialProCharacterCounting = false;
...
jQuery(document).ready(function ($) {
	...
	$(wp_to_social_pro.status_form).on(
		...
		'select.schedule',
		function () {
			wpToSocialProUpdateScheduleOptions(
	...
	$('body').on('wpzinc-media-library-attachment-added', function () {
		...
		if (
			typeof $(wp_to_social_pro.status_form).data('profile-id') ===
			'undefined'
		) {
			return;
		}
	 ...
	 */
	$('form.wpzinc-social-publisher-pro').on('submit', function (e) {
		// Don't submit form.
		e.preventDefault();
		...
		wpToSocialProSaveStatuses(
			wp_to_social_pro.post_type,
			$(
