...
class Admin_UI {
	...
	public function register_import_export_menu( $parent_slug = '' ) {
		...
		add_submenu_page( $slug, __( 'Import & Export', 'wpzinc-social-publisher-for-postiz' ), __( 'Import & Export', 'wpzinc-social-publisher-for-postiz' ), $minimum_capability, $this->plugin->name . '-import-export', array( $this, 'import_export_screen' ) );
	...
	public function add_action_link( $links ) {
		...
		if ( $this->get_upgrade_url( 'plugins' ) ) {
			$links[] = '<a href="' . esc_attr( $this->get_upgrade_url( 'plugins' ) ) . '" rel="noopener" target="_blank">' . __( 'Upgrade', 'wpzinc-social-publisher-for-postiz' ) . '</a>';
		}
	...
	public function maybe_display_footer_review_request( $text ) {
		...
		return sprintf(
			...
			__( 'Please rate %1$s %2$s on %3$s to help us grow %4$s. Thanks!', 'wpzinc-social-publisher-for-postiz' ),
	...
	private function import() {
		...
		if ( isset( $_FILES['import']['error'] ) && $_FILES['import']['error'] !== 0 ) {
			$this->error_message = __( 'Error when uploading file.', 'wpzinc-social-publisher-for-postiz' );
			return;
		}
		...
		switch ( $_FILES['import']['type'] ) {
			...
			case 'application/zip':
				...
				if ( $zip->open( sanitize_text_field( wp_unslash( $_FILES['import']['tmp_name'] ) ) ) !== true ) {
					$this->error_message = __( 'Could not extract the supplied ZIP file.', 'wpzinc-social-publisher-for-postiz' );
					return;
				}
		...
		if ( ! is_array( $import ) ) {
			$this->error_message = __( 'Supplied file is not a valid JSON settings file, or has become corrupt.', 'wpzinc-social-publisher-for-postiz' );
			return;
		}
		...
		$this->message = __( 'Settings imported.', 'wpzinc-social-publisher-for-postiz' );
