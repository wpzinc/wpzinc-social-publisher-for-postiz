 ...
 * @since 	3.9.6
 */
function wpZincSocialReinitAutosize() {
	(function ($) {
		// Bail if no autosize instances exist.
		if (
			$('.wpzinc-autosize-js', $(wpzinc_social.status_form)).length === 0
		) {
			return;
		}
 ...
 * @since 	3.9.6
 */
function wpZincSocialReinitAutocomplete() {
 ...
 * @since 	3.9.6
 */
function wpZincSocialInitTags() {
	(function ($) {
		// Bail if no tag instances exist.
		if ($('select.tags', $(wpzinc_social.status_form)).length === 0) {
			return;
		}

		$('select.tags', $(wpzinc_social.status_form)).each(function () {
			$(this).on('change.wpzinc-social', function () {
				// Insert tag into required textarea.
				let tag = $(this).val();
				const textarea = $(this).attr('data-textarea');
				const option = $('option:selected', $(this));
				const status = $(this).closest('.wpzinc-social-status-form');
 ...
 */
function wpZincSocialInitSelectize(selector, profile_id, action, status_index) {
	(function ($) {
				...
				load(query, callback) {
					...
					$.ajax({
						...
						data: {
							...
							nonce: wpzinc_social[this.$input.data('nonce-key')],
							q: query,
							page: 10,
						},
						error() {
 ...
 */
function wpZincSocialDestroySelectize(selector) {
	(function ($) {
		$('.wpzinc-selectize', $(selector)).each(function () {
			if (this.selectize) {
 ...
 */
function wpZincSocialReindexStatuses(statuses_container) {
	(function ($) {
 ...
 * @param {string} action Action (publish,update,repost,bulk_publish)
 */
function wpZincSocialUpdateScheduleOptions(action) {
	(function ($) {
		// Bail if no schedule dropdowns exist.
		if ($('select.schedule', $(wpzinc_social.status_form)).length === 0) {
			return;
		}

		// Show / hide schedule options.
		switch ($('select.schedule', $(wpzinc_social.status_form)).val()) {
			case 'custom':
				$('div.schedule', $(wpzinc_social.status_form)).show();
				$('span.schedule', $(wpzinc_social.status_form)).show();
				$('span.hours_mins_secs', $(wpzinc_social.status_form)).show();
				$('span.relative', $(wpzinc_social.status_form)).hide();
				$('span.custom', $(wpzinc_social.status_form))
					.text('after ' + action)
					.show();
				$('span.custom_field', $(wpzinc_social.status_form)).hide();
				$(
					'span.the_events_calendar',
					$(wpzinc_social.status_form)
				).hide();
				$('span.events_manager', $(wpzinc_social.status_form)).hide();
				$(
					'span.modern_events_calendar',
					$(wpzinc_social.status_form)
				).hide();
				$('span.specific', $(wpzinc_social.status_form)).hide();
				break;

			case 'custom_relative':
				$('div.schedule', $(wpzinc_social.status_form)).show();
				$('span.schedule', $(wpzinc_social.status_form)).show();
				$('span.hours_mins_secs', $(wpzinc_social.status_form)).hide();
				$('span.relative', $(wpzinc_social.status_form)).show();
				$('span.custom', $(wpzinc_social.status_form))
					.text('after ' + action)
					.show();
				$('span.custom_field', $(wpzinc_social.status_form)).hide();
			...
			case 'custom_field':
				...
				$('span.hours_mins_secs', $(wpzinc_social.status_form)).show();
				$('span.relative', $(wpzinc_social.status_form)).hide();
				$('span.custom', $(wpzinc_social.status_form)).text('').hide();
				$('span.custom_field', $(wpzinc_social.status_form)).show();
				$(
					'span.the_events_calendar',
					$(wpzinc_social.status_form)
				).hide();
			...
			case '_event_end_date':
				...
				$('span.events_manager', $(wpzinc_social.status_form)).show();
				$(
					'span.modern_events_calendar',
					$(wpzinc_social.status_form)
				).hide();
			...
			case 'specific':
				...
				$('span.specific', $(wpzinc_social.status_form)).show();
				break;

			default:
				// Hide additonal schedule options.
				$('div.schedule', $(wpzinc_social.status_form)).hide();
				$('span.schedule', $(wpzinc_social.status_form)).hide();
				$('span.hours_mins_secs', $(wpzinc_social.status_form)).hide();
				$('span.relative', $(wpzinc_social.status_form)).hide();
				$('span.custom', $(wpzinc_social.status_form)).text('').hide();
				$('span.custom_field', $(wpzinc_social.status_form)).hide();
 ...
 * @since 	4.2.0
 */
function wpZincSocialUpdateImageOptions() {
	(function ($) {
		// Bail if no image dropdowns exist.
		if ($('select.image', $(wpzinc_social.status_form)).length === 0) {
			return;
		}
 ...
 * @since 	4.9.0
 */
function wpZincSocialUpdateGoogleBusinessOptions() {
	(function ($) {
		// Bail if no Google Business Post Type dropdown exists.
		if (
			$('select#googlebusiness_post_type', $(wpzinc_social.status_form))
				.length === 0
		) {
			return;
		}
		...
		// Hide all table rows.
		$('tr.whats_new', $(wpzinc_social.status_form)).hide();
		$('tr.event', $(wpzinc_social.status_form)).hide();
		$('tr.offer', $(wpzinc_social.status_form)).hide();
 ...
 */
function wpZincSocialUpdatePostTypeOptions(profile) {
	(function ($) {
		// Disable all options.
		$('option', $('select.post_type', $(wpzinc_social.status_form))).attr(
			'disabled',
			true
		);

		// Enable options based on the profile service.
		switch (profile.service) {
			case 'instagram':
				$(
					'option[value="image"]',
					$('select.post_type', $(wpzinc_social.status_form))
				).attr('disabled', false);
				$(
					'option[value="story"]',
					$('select.post_type', $(wpzinc_social.status_form))
		...
		if (
			$('select.post_type', $(wpzinc_social.status_form)).val() ===
			'disabled'
		) {
			$('select.post_type', $(wpzinc_social.status_form)).val(
				$(
 ...
 * @since 	6.0.0
 */
function wpZincSocialUpdateStatusSections() {
	(function ($) {
		// Hide all sections.
		$('.wpzinc-option.link', $(wpzinc_social.status_form)).hide();
		$('.wpzinc-option.images', $(wpzinc_social.status_form)).hide();
		$('.wpzinc-option.pinterest', $(wpzinc_social.status_form)).hide();
		$('.wpzinc-option.googlebusiness', $(wpzinc_social.status_form)).hide();

		// Show sections based on the chosen status post type.
		switch ($('select.post_type', $(wpzinc_social.status_form)).val()) {
			case 'link':
				$('.wpzinc-option.link', $(wpzinc_social.status_form)).show();
				$('.wpzinc-option.images', $(wpzinc_social.status_form)).hide();
				break;
			...
			case 'story':
				$('.wpzinc-option.link', $(wpzinc_social.status_form)).hide();
				$('.wpzinc-option.images', $(wpzinc_social.status_form)).show();
				break;
 ...
 * @param {string} action     Action (publish,update,repost,bulk_publish).
 */
function wpZincSocialAddStatus(profile_id, action) {
	(function ($) {
		// Get last status row.
		...
		// Reindex statuses.
		wpZincSocialReindexStatuses(statuses_container);

		// Populate hidden field with all statuses' data.
		wpZincSocialUpdateStatuses();
	})(jQuery);
}
 ...
 */
function wpZincSocialEditStatus(
	profile_id,
	profile,
	action,
	status_index,
	...
	// Destroy selectize.
	wpZincSocialDestroySelectize(wpzinc_social.status_form);

	// Populate form values.
	wpZincSocialPopulateStatusForm(
		profile_id,
		profile,
		action,
		status_index,
	...
	wpZincSocialUpdatePostTypeOptions(profile);

	// Update schedule options.
	wpZincSocialUpdateScheduleOptions(action);

	// Update image options.
	wpZincSocialUpdateImageOptions();

	// Update status sections to display, depending on the status' post type.
	wpZincSocialUpdateStatusSections();

	// Update Google Business options.
	wpZincSocialUpdateGoogleBusinessOptions();

	// Display form.
	wpZincSocialDisplayStatusForm(profile_id, action, status_index);

	// Reinit autosize.
	wpZincSocialReinitAutosize();

	// Reinit autocomplete.
	wpZincSocialReinitAutocomplete();

	// Init selectize.
	wpZincSocialInitSelectize(
		wpzinc_social.status_form,
		profile_id,
		action,
		status_index
 ...
 * @param {number} status_index Zero based index of this status relative to all statuses for the Profile ID and Action.
 */
function wpZincSocialDeleteStatus(profile_id, action, status_index) {
	(function ($) {
		// Confirm deletion.
		const result = confirm(wpzinc_social.delete_status_message);
		if (!result) {
			return;
		}
 ...
 * @param {number} status_index Zero based index of this status relative to all statuses for the Profile ID and Action.
 */
function wpZincSocialUpdateStatus(profile_id, action, status_index) {
	(function ($) {
		// Get row.
		const row = $(
			'div.statuses[data-profile-id="' +
		...
		$.ajax({
			...
			async: true,
			data: {
				action: wpzinc_social.get_status_row_action,
				nonce: wpzinc_social.get_status_row_nonce,
				post_type: wpzinc_social.post_type,
				post_action: action,
				status: JSON.stringify(status),
			},
			error(xhr) {
				alert(
					'wpZincSocialUpdateStatus(): error: ' +
						xhr.status +
						' ' +
						xhr.statusText
				);
 ...
 * @param {Object} status       Status.
 */
function wpZincSocialPopulateStatusForm(
	profile_id,
	profile,
	action,
	status_index,
	...
	(function ($) {
		...
		$('input, select, textarea', $(wpzinc_social.status_form)).each(
			function () {
				...
				// Remove prepended Plugin Name from Field Name.
				field = field.replace(wpzinc_social.plugin_name + '_', '');
				...
				switch (field) {
					...
					case 'pinterest[board]':
						...
						// Populate and show either the <select> or <input> pinterest[board] field.
						if (typeof profile.subprofiles !== typeof undefined) {
							...
							if (!Object.keys(profile.subprofiles).length) {
								...
								for (const key in profile.subprofiles) {
									$(
										'select[name="' +
											wpzinc_social.plugin_name +
											'_' +
										...
										$('<option></option>')
											.attr(
												'value',
												profile.subprofiles[key].id
											)
											.text(profile.subprofiles[key].name)
									);
								}
		...
		$('input, select', $(wpzinc_social.status_form)).conditional();
	})(jQuery);
}
 ...
 * @param {number} status_index Zero based index of this status relative to all statuses for the Profile ID and Action.
 */
function wpZincSocialDisplayStatusForm(profile_id, action, status_index) {
	(function ($) {
		// Get row.
		const row = $(
				'div.statuses[data-profile-id="' +
		...
		$('td', status_form_container_row).append(
			$(wpzinc_social.status_form).removeClass('hidden')
		);
 ...
 * @since 	4.4.0
 */
function wpZincSocialSaveAndHideStatusForm() {
	(function ($) {
		// If the status form is visible, save its values now.
		if ($('div.statuses div.wpzinc-social-status-form').length > 0) {
			wpZincSocialUpdateStatus(
				$(
					'div.statuses tr.status-form-container div.wpzinc-social-status-form'
				).data('profile-id'),
				$(
					'div.statuses tr.status-form-container div.wpzinc-social-status-form'
				).data('action'),
		...
		$(wpzinc_social.status_form).data('action', '').attr('data-action', '');
		$(wpzinc_social.status_form)
			.data('status-index', '')
			.attr('data-status-index', '');

		// Move form into container.
		$(wpzinc_social.status_form_container).append(
			$(wpzinc_social.status_form)
		);
	})(jQuery);
}
 ...
 * @since 	4.4.0
 */
function wpZincSocialClearStatusForm() {
	(function ($) {
		// Clear all values.
		$('input, select, textarea', $(wpzinc_social.status_form)).each(
			function () {
 ...
 * @since 	4.4.0
 */
function wpZincSocialGetStatuses() {
	const statuses = {};
 ...
 * @since 	4.4.0
 */
function wpZincSocialUpdateStatuses() {
	let statuses;

	(function ($) {
		// Get statuses.
		statuses = wpZincSocialGetStatuses();
 ...
 */
function wpZincSocialSaveStatuses(post_type, tab) {
	let statuses;
	...
	(function ($) {
		...
		wpzinc_modal_open(wpzinc_social.save_statuses_modal.title, '');
		...
		$.ajax({
			...
			async: true,
			data: {
				action: wpzinc_social.save_statuses_action,
				nonce: wpzinc_social.save_statuses_nonce,
				post_type,
				statuses: JSON.stringify(statuses),
			},
			error(xhr) {
				wpzinc_modal_show_error_message_and_exit(
					'wpZincSocialSaveStatuses(): error: ' +
						xhr.status +
						' ' +
						xhr.statusText
				);
			...
			success(result) {
				...
				wpzinc_modal_show_success_and_exit(
					wpzinc_social.save_statuses_modal.title_success
				);
			},
		});
	})(jQuery);
 ...
 */
function wpZincSocialSavePostStatuses(
	post_id,
	override,
	featured_image,
	additional_images
...
let wpZincSocialCharacterCounting = false;
 ...
 */
function wpZincSocialCharacterCount(textarea) {
	(function ($) {
		// If we're currently running an AJAX request, don't run another one.
		if (wpZincSocialCharacterCounting) {
			return;
		}

		// Set a flag so we know we're performing an AJAX request.
		wpZincSocialCharacterCounting = true;

		// Send an AJAX request to fetch the parsed statuses and character counts for each status.
		$.post(
			wpzinc_social.ajax,
			{
				action: wpzinc_social.character_count_action,
				post_id: wpzinc_social.post_id,
				status: $(textarea).val(),
				nonce: wpzinc_social.character_count_nonce,
			},
			function (response) {
				...
				setTimeout(function () {
					wpZincSocialCharacterCounting = false;
				}, 3000);
			}
		);
	})(jQuery);
 ...
 * @since 	4.5.5
 */
function wpZincSocialDisableNotSavedPrompt() {
...
jQuery(document).ready(function ($) {
	// Tags dropdown.
	wpZincSocialInitTags();

	// Status sections.
	$(wpzinc_social.status_form).on(
		'change.' + wpzinc_social.status_form,
		'select.post_type',
		function () {
			wpZincSocialUpdateStatusSections();
		}
	...
	$(wpzinc_social.status_form).on(
		...
		'select.schedule',
		function () {
			wpZincSocialUpdateScheduleOptions(
	...
	$('input.enable', $('#profiles-container')).on('change', function () {
		...
		wpZincSocialSaveAndHideStatusForm();

		wpZincSocialClearStatusForm();

		wpZincSocialUpdateStatuses();

		wpZincSocialEnableNotSavedPrompt();
	});
	...
	$(wpzinc_social.status_form).on(
		...
		function () {
			wpZincSocialUpdateStatus(
				$(wpzinc_social.status_form).data('profile-id'),
				$(wpzinc_social.status_form).data('action'),
				$(wpzinc_social.status_form).data('status-index')
			);
	...
	$('body').on('wpzinc-media-library-attachment-added', function () {
		...
		if (
			typeof $(wpzinc_social.status_form).data('profile-id') ===
			'undefined'
		) {
			return;
		}
	 ...
	 * @since 	3.0.0
	 */
	if (wpzinc_social.post_id > 0) {
		$('textarea.message', $(wpzinc_social.status_form)).on(
			'keyup change',
			function () {
				wpZincSocialCharacterCount(this);
			}
		);
	}
	 ...
	 */
	$('form.wpzinc-social').on('submit', function (e) {
		// Don't submit form.
		e.preventDefault();

		// Disable the not saved prompt, as we're about to save.
		wpZincSocialDisableNotSavedPrompt();

		// Populate hidden field with all statuses' data.
		wpZincSocialUpdateStatuses();

		// Save Post Type statuses.
		wpZincSocialSaveStatuses(
			wpzinc_social.post_type,
			$(
	 ...
	 */
	$('button.' + wpzinc_social.plugin_name + '-save-post-statuses').on(
		'click',
		function (e) {
			// Don't submit form.
			e.preventDefault();
			...
			// Save Post statuses.
			wpZincSocialSavePostStatuses(
				wpzinc_social.post_id,
				$(
					'select[name="' + wpzinc_social.plugin_name + '[override]"]'
				).val(),
				$(
					'input[name="' +
						wpzinc_social.plugin_name +
