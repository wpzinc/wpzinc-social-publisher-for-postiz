			...
			switch ( $oauth_error ) {
				...
				case 'access_denied':
					$this->base->get_class( 'notices' )->add_error_notice(
						sprintf(
							/* translators: %1$s: Social Media Service Name (Buffer, Hootsuite), %2$s: Social Media Service Name (Buffer, Hootsuite) */
							__( 'You did not grant our Plugin access to your %1$s account. We are unable to post to %2$s until you do this. Please click on the Authorize Plugin button.', 'wpzinc-social-publisher-for-postiz' ),
							$this->base->plugin->account,
							$this->base->plugin->account
						)
					);
					...
					$this->base->get_class( 'notices' )->add_error_notice(
						sprintf(
							...
							sprintf(
								...
								__( 'We were unable to complete authentication with %s.  Please try again, or', 'wpzinc-social-publisher-for-postiz' ),
								$this->base->plugin->account
							),
							esc_html( $this->base->plugin->support_url ),
							__( 'contact us for support', 'wpzinc-social-publisher-for-postiz' )
						)
					);
					break;
				...
				case 'expired_token':
					$this->base->get_class( 'notices' )->add_error_notice(
						sprintf(
							...
							__( 'The oAuth process has expired.  Please try again, or', 'wpzinc-social-publisher-for-postiz' ),
							esc_html( $this->base->plugin->support_url ),
							__( 'contact us for support', 'wpzinc-social-publisher-for-postiz' ),
							__( 'if this issue persists.', 'wpzinc-social-publisher-for-postiz' )
						)
					);
					break;
			...
			if ( strpos( $account['access_token'], '2/' ) === 0 ) {
				$this->base->get_class( 'notices' )->add_error_notice(
					sprintf(
						...
						__( '%1$s uses a new API. Please click the `Reconnect` button at %2$s Settings > Authentication to reconnect your account. You won\'t need to do this again.', 'wpzinc-social-publisher-for-postiz' ),
						$this->base->plugin->displayName,
						$this->base->plugin->account
					)
				);
